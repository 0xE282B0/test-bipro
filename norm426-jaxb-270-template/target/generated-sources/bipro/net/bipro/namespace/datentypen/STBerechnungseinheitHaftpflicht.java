
package net.bipro.namespace.datentypen;

import javax.annotation.Generated;
import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for ST_BerechnungseinheitHaftpflicht.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="ST_BerechnungseinheitHaftpflicht"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="001"/&gt;
 *     &lt;enumeration value="002"/&gt;
 *     &lt;enumeration value="003"/&gt;
 *     &lt;enumeration value="004"/&gt;
 *     &lt;enumeration value="005"/&gt;
 *     &lt;enumeration value="006"/&gt;
 *     &lt;enumeration value="007"/&gt;
 *     &lt;enumeration value="008"/&gt;
 *     &lt;enumeration value="010"/&gt;
 *     &lt;enumeration value="011"/&gt;
 *     &lt;enumeration value="012"/&gt;
 *     &lt;enumeration value="014"/&gt;
 *     &lt;enumeration value="015"/&gt;
 *     &lt;enumeration value="016"/&gt;
 *     &lt;enumeration value="017"/&gt;
 *     &lt;enumeration value="018"/&gt;
 *     &lt;enumeration value="019"/&gt;
 *     &lt;enumeration value="020"/&gt;
 *     &lt;enumeration value="021"/&gt;
 *     &lt;enumeration value="022"/&gt;
 *     &lt;enumeration value="023"/&gt;
 *     &lt;enumeration value="024"/&gt;
 *     &lt;enumeration value="025"/&gt;
 *     &lt;enumeration value="026"/&gt;
 *     &lt;enumeration value="027"/&gt;
 *     &lt;enumeration value="028"/&gt;
 *     &lt;enumeration value="029"/&gt;
 *     &lt;enumeration value="030"/&gt;
 *     &lt;enumeration value="031"/&gt;
 *     &lt;enumeration value="032"/&gt;
 *     &lt;enumeration value="033"/&gt;
 *     &lt;enumeration value="034"/&gt;
 *     &lt;enumeration value="035"/&gt;
 *     &lt;enumeration value="036"/&gt;
 *     &lt;enumeration value="038"/&gt;
 *     &lt;enumeration value="039"/&gt;
 *     &lt;enumeration value="040"/&gt;
 *     &lt;enumeration value="041"/&gt;
 *     &lt;enumeration value="043"/&gt;
 *     &lt;enumeration value="044"/&gt;
 *     &lt;enumeration value="046"/&gt;
 *     &lt;enumeration value="047"/&gt;
 *     &lt;enumeration value="048"/&gt;
 *     &lt;enumeration value="049"/&gt;
 *     &lt;enumeration value="050"/&gt;
 *     &lt;enumeration value="051"/&gt;
 *     &lt;enumeration value="052"/&gt;
 *     &lt;enumeration value="054"/&gt;
 *     &lt;enumeration value="055"/&gt;
 *     &lt;enumeration value="056"/&gt;
 *     &lt;enumeration value="057"/&gt;
 *     &lt;enumeration value="059"/&gt;
 *     &lt;enumeration value="060"/&gt;
 *     &lt;enumeration value="061"/&gt;
 *     &lt;enumeration value="062"/&gt;
 *     &lt;enumeration value="063"/&gt;
 *     &lt;enumeration value="064"/&gt;
 *     &lt;enumeration value="065"/&gt;
 *     &lt;enumeration value="066"/&gt;
 *     &lt;enumeration value="068"/&gt;
 *     &lt;enumeration value="070"/&gt;
 *     &lt;enumeration value="071"/&gt;
 *     &lt;enumeration value="072"/&gt;
 *     &lt;enumeration value="073"/&gt;
 *     &lt;enumeration value="074"/&gt;
 *     &lt;enumeration value="075"/&gt;
 *     &lt;enumeration value="076"/&gt;
 *     &lt;enumeration value="077"/&gt;
 *     &lt;enumeration value="078"/&gt;
 *     &lt;enumeration value="079"/&gt;
 *     &lt;enumeration value="080"/&gt;
 *     &lt;enumeration value="081"/&gt;
 *     &lt;enumeration value="082"/&gt;
 *     &lt;enumeration value="083"/&gt;
 *     &lt;enumeration value="084"/&gt;
 *     &lt;enumeration value="085"/&gt;
 *     &lt;enumeration value="086"/&gt;
 *     &lt;enumeration value="087"/&gt;
 *     &lt;enumeration value="088"/&gt;
 *     &lt;enumeration value="089"/&gt;
 *     &lt;enumeration value="091"/&gt;
 *     &lt;enumeration value="092"/&gt;
 *     &lt;enumeration value="093"/&gt;
 *     &lt;enumeration value="094"/&gt;
 *     &lt;enumeration value="095"/&gt;
 *     &lt;enumeration value="096"/&gt;
 *     &lt;enumeration value="097"/&gt;
 *     &lt;enumeration value="099"/&gt;
 *     &lt;enumeration value="100"/&gt;
 *     &lt;enumeration value="101"/&gt;
 *     &lt;enumeration value="102"/&gt;
 *     &lt;enumeration value="103"/&gt;
 *     &lt;enumeration value="104"/&gt;
 *     &lt;enumeration value="105"/&gt;
 *     &lt;enumeration value="107"/&gt;
 *     &lt;enumeration value="108"/&gt;
 *     &lt;enumeration value="109"/&gt;
 *     &lt;enumeration value="110"/&gt;
 *     &lt;enumeration value="111"/&gt;
 *     &lt;enumeration value="112"/&gt;
 *     &lt;enumeration value="113"/&gt;
 *     &lt;enumeration value="115"/&gt;
 *     &lt;enumeration value="116"/&gt;
 *     &lt;enumeration value="117"/&gt;
 *     &lt;enumeration value="118"/&gt;
 *     &lt;enumeration value="120"/&gt;
 *     &lt;enumeration value="121"/&gt;
 *     &lt;enumeration value="122"/&gt;
 *     &lt;enumeration value="123"/&gt;
 *     &lt;enumeration value="124"/&gt;
 *     &lt;enumeration value="126"/&gt;
 *     &lt;enumeration value="127"/&gt;
 *     &lt;enumeration value="128"/&gt;
 *     &lt;enumeration value="129"/&gt;
 *     &lt;enumeration value="800"/&gt;
 *     &lt;enumeration value="801"/&gt;
 *     &lt;enumeration value="802"/&gt;
 *     &lt;enumeration value="803"/&gt;
 *     &lt;enumeration value="804"/&gt;
 *     &lt;enumeration value="805"/&gt;
 *     &lt;enumeration value="806"/&gt;
 *     &lt;enumeration value="807"/&gt;
 *     &lt;enumeration value="808"/&gt;
 *     &lt;enumeration value="809"/&gt;
 *     &lt;enumeration value="810"/&gt;
 *     &lt;enumeration value="811"/&gt;
 *     &lt;enumeration value="812"/&gt;
 *     &lt;enumeration value="813"/&gt;
 *     &lt;enumeration value="814"/&gt;
 *     &lt;enumeration value="815"/&gt;
 *     &lt;enumeration value="816"/&gt;
 *     &lt;enumeration value="817"/&gt;
 *     &lt;enumeration value="818"/&gt;
 *     &lt;enumeration value="999"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(name = "ST_BerechnungseinheitHaftpflicht")
@XmlEnum
@Generated(value = "com.sun.tools.xjc.Driver", date = "2021-03-19T04:27:26+01:00", comments = "JAXB RI v2.3.2")
public enum STBerechnungseinheitHaftpflicht {


    /**
     * Personen, Person
     * 
     */
    @XmlEnumValue("001")
    PERSONEN("001"),

    /**
     * Lohn-/Gehaltssumme
     * 
     */
    @XmlEnumValue("002")
    LOHN_GEHALTSSUMME("002"),

    /**
     * Umsatz
     * 
     */
    @XmlEnumValue("003")
    UMSATZ("003"),

    /**
     * Anlagen, Anlage
     * 
     */
    @XmlEnumValue("004")
    ANLAGEN("004"),

    /**
     * Strahler
     * 
     */
    @XmlEnumValue("005")
    STRAHLER("005"),

    /**
     * Bausumme
     * 
     */
    @XmlEnumValue("006")
    BAUSUMME("006"),

    /**
     * Beitrag
     * 
     */
    @XmlEnumValue("007")
    BEITRAG("007"),

    /**
     * Tanks, Tank
     * 
     */
    @XmlEnumValue("008")
    TANKS("008"),

    /**
     * Apparate, Apparat
     * 
     */
    @XmlEnumValue("010")
    APPARATE("010"),

    /**
     * Megawatt / H
     * 
     */
    @XmlEnumValue("011")
    MEGAWATT("011"),

    /**
     * km Laenge
     * 
     */
    @XmlEnumValue("012")
    KM_LAENGE("012"),

    /**
     * Anschluesse, Anschluss
     * 
     */
    @XmlEnumValue("014")
    ANSCHLUESSE("014"),

    /**
     * CBM
     * 
     */
    @XmlEnumValue("015")
    CBM("015"),

    /**
     * CBM Inhalt
     * 
     */
    @XmlEnumValue("016")
    CBM_INHALT("016"),

    /**
     * Schlachtungen, Schlachtung
     * 
     */
    @XmlEnumValue("017")
    SCHLACHTUNGEN("017"),

    /**
     * Faecher, Fach
     * 
     */
    @XmlEnumValue("018")
    FAECHER("018"),

    /**
     * Agenturen, Agentur
     * 
     */
    @XmlEnumValue("019")
    AGENTUREN("019"),

    /**
     * Hetar (tariflich)
     * 
     */
    @XmlEnumValue("020")
    HEKTAR_TARIFLICH("020"),

    /**
     * Hektar
     * 
     */
    @XmlEnumValue("021")
    HEKTAR("021"),

    /**
     * Spritzen, Spritze
     * 
     */
    @XmlEnumValue("022")
    SPRITZEN("022"),

    /**
     * Tiere, Tier
     * 
     */
    @XmlEnumValue("023")
    TIERE("023"),

    /**
     * Raeume, Raum
     * 
     */
    @XmlEnumValue("024")
    RAEUME("024"),

    /**
     * Versicherungssumme
     * 
     */
    @XmlEnumValue("025")
    VERSICHERUNGSSUMME("025"),

    /**
     * Geraete, Geraet
     * 
     */
    @XmlEnumValue("026")
    GERAETE("026"),

    /**
     * Pferde, Pferd
     * 
     */
    @XmlEnumValue("027")
    PFERDE("027"),

    /**
     * Bahnen, Bahn
     * 
     */
    @XmlEnumValue("028")
    BAHNEN("028"),

    /**
     * Betten, Bett
     * 
     */
    @XmlEnumValue("029")
    BETTEN("029"),

    /**
     * Ponys, Pony
     * 
     */
    @XmlEnumValue("030")
    PONYS("030"),

    /**
     * Hengste, Hengst
     * 
     */
    @XmlEnumValue("031")
    HENGSTE("031"),

    /**
     * Stiere, Stier
     * 
     */
    @XmlEnumValue("032")
    STIERE("032"),

    /**
     * Hunde, Hund
     * 
     */
    @XmlEnumValue("033")
    HUNDE("033"),

    /**
     * Voelker, Volk
     * 
     */
    @XmlEnumValue("034")
    VOELKER("034"),

    /**
     * Schafe, Schaf
     * 
     */
    @XmlEnumValue("035")
    SCHAFE("035"),

    /**
     * Bagger
     * 
     */
    @XmlEnumValue("036")
    BAGGER("036"),

    /**
     * Honorar
     * 
     */
    @XmlEnumValue("038")
    HONORAR("038"),

    /**
     * Zimmer
     * 
     */
    @XmlEnumValue("039")
    ZIMMER("039"),

    /**
     * Fremdenzimmer
     * 
     */
    @XmlEnumValue("040")
    FREMDENZIMMER("040"),

    /**
     * Schiess-Staende, Schiess-Stand
     * 
     */
    @XmlEnumValue("041")
    SCHIESSSTAENDE("041"),

    /**
     * Bloecke, Block
     * 
     */
    @XmlEnumValue("043")
    BLOECKE("043"),

    /**
     * Plaetze, Platz
     * 
     */
    @XmlEnumValue("044")
    PLAETZE("044"),

    /**
     * Saele, Saal
     * 
     */
    @XmlEnumValue("046")
    SAELE("046"),

    /**
     * Wirtschaften, Wirtschaft
     * 
     */
    @XmlEnumValue("047")
    WIRTSCHAFTEN("047"),

    /**
     * qm Flaeche
     * 
     */
    @XmlEnumValue("048")
    QM_FLAECHE("048"),

    /**
     * Lenker
     * 
     */
    @XmlEnumValue("049")
    LENKER("049"),

    /**
     * qm
     * 
     */
    @XmlEnumValue("050")
    QM("050"),

    /**
     * KFZ
     * 
     */
    @XmlEnumValue("051")
    KFZ("051"),

    /**
     * Automaten, Automat
     * 
     */
    @XmlEnumValue("052")
    AUTOMATEN("052"),

    /**
     * Spielfelder, Spielfeld
     * 
     */
    @XmlEnumValue("054")
    SPIELFELDER("054"),

    /**
     * Einstellplaetze, Einstellplatz
     * 
     */
    @XmlEnumValue("055")
    EINSTELLPLAETZE("055"),

    /**
     * Saeuglinge, Saeugling
     * 
     */
    @XmlEnumValue("056")
    SAEUGLINGE("056"),

    /**
     * Schwestern, Schwester
     * 
     */
    @XmlEnumValue("057")
    SCHWESTERN("057"),

    /**
     * Probanden, Proband
     * 
     */
    @XmlEnumValue("059")
    PROBANDEN("059"),

    /**
     * Besucher
     * 
     */
    @XmlEnumValue("060")
    BESUCHER("060"),

    /**
     * Badeeinrichtungen, Badeeinrichtung
     * 
     */
    @XmlEnumValue("061")
    BADEEINRICHTUNGEN("061"),

    /**
     * Fahrzeuge, Fahrzeug
     * 
     */
    @XmlEnumValue("062")
    FAHRZEUGE("062"),

    /**
     * m Frontlaenge
     * 
     */
    @XmlEnumValue("063")
    M_FRONTLAENGE("063"),

    /**
     * Wagen
     * 
     */
    @XmlEnumValue("064")
    WAGEN("064"),

    /**
     * Uebergaenge, Uebergang
     * 
     */
    @XmlEnumValue("065")
    UEBERGAENGE("065"),

    /**
     * Kreuzungen, Kreuzung
     * 
     */
    @XmlEnumValue("066")
    KREUZUNGEN("066"),

    /**
     * Waagen, Waage
     * 
     */
    @XmlEnumValue("068")
    WAAGEN("068"),

    /**
     * Wagen / Container
     * 
     */
    @XmlEnumValue("070")
    WAGEN_CONTAINER("070"),

    /**
     * Einnahmen
     * 
     */
    @XmlEnumValue("071")
    EINNAHMEN("071"),

    /**
     * KFZ / Geraete, KFZ / Geraet
     * 
     */
    @XmlEnumValue("072")
    KFZ_GERAETE("072"),

    /**
     * Kraene, Kran
     * 
     */
    @XmlEnumValue("073")
    KRAENE("073"),

    /**
     * Winden, Winde
     * 
     */
    @XmlEnumValue("074")
    WINDE("074"),

    /**
     * Vorrichtungen, Vorrichtung
     * 
     */
    @XmlEnumValue("075")
    VORRICHTUNGEN("075"),

    /**
     * Teilnehmer
     * 
     */
    @XmlEnumValue("076")
    TEILNEHMER("076"),

    /**
     * Tafeln, Tafel
     * 
     */
    @XmlEnumValue("077")
    TAFELN("077"),

    /**
     * Saeulen, Saeule
     * 
     */
    @XmlEnumValue("078")
    SAEULEN("078"),

    /**
     * Abonnenten, Abonnent
     * 
     */
    @XmlEnumValue("079")
    ABONNENTEN("079"),

    /**
     * KFZ-Einstellplaetze, KFZ-Einstellplatz
     * 
     */
    @XmlEnumValue("080")
    KFZ_EINSTELLPLAETZE("080"),

    /**
     * Einstellplaetze fuer Fahrraeder, Einstellplatz fuer Fahrrad
     * 
     */
    @XmlEnumValue("081")
    EINSTELLPLAETZE_FUER_FAHRRAEDER("081"),

    /**
     * Schliessfaecher, Schliessfach
     * 
     */
    @XmlEnumValue("082")
    SCHLIESSFAECHER("082"),

    /**
     * Zapfstellen, Zapfstelle
     * 
     */
    @XmlEnumValue("083")
    ZAPFSTELLE("083"),

    /**
     * Aufbewahrungsscheine, Aufbewahrungsschein
     * 
     */
    @XmlEnumValue("084")
    AUFBEWAHRUNGSSCHEINE("084"),

    /**
     * Mitglieder, Mitglied
     * 
     */
    @XmlEnumValue("085")
    MITGLIEDER("085"),

    /**
     * Boote, Boot
     * 
     */
    @XmlEnumValue("086")
    BOOTE("086"),

    /**
     * qm Segelflaeche
     * 
     */
    @XmlEnumValue("087")
    QM_SEGELFLAECHE("087"),

    /**
     * m Bahnlaenge
     * 
     */
    @XmlEnumValue("088")
    M_BAHNLAENGE("088"),

    /**
     * Schanzen, Schanze
     * 
     */
    @XmlEnumValue("089")
    SCHANZEN("089"),

    /**
     * Kinder, Kind
     * 
     */
    @XmlEnumValue("091")
    KINDER("091"),

    /**
     * Schuetzen, Schuetze
     * 
     */
    @XmlEnumValue("092")
    SCHUETZEN("092"),

    /**
     * Tiere / Boote / Geraete, Tier / Boot / Geraet
     * 
     */
    @XmlEnumValue("093")
    TIERE_BOOTE_GERAETE("093"),

    /**
     * Reiter
     * 
     */
    @XmlEnumValue("094")
    REITER("094"),

    /**
     * Aussteller
     * 
     */
    @XmlEnumValue("095")
    AUSSTELLER("095"),

    /**
     * Teilnehmer / Besucher
     * 
     */
    @XmlEnumValue("096")
    TEILNEHMER_BESUCHER("096"),

    /**
     * Boeller
     * 
     */
    @XmlEnumValue("097")
    BOELLER("097"),

    /**
     * Mietwert
     * 
     */
    @XmlEnumValue("099")
    MIETWERT("099"),

    /**
     * Wohnungen, Wohnung
     * 
     */
    @XmlEnumValue("100")
    WOHNUNGEN("100"),

    /**
     * Grundstuecke, Grundstueck
     * 
     */
    @XmlEnumValue("101")
    GRUNDSTUECKE("101"),

    /**
     * m Strassenfront
     * 
     */
    @XmlEnumValue("102")
    M_STRASSENFRONT("102"),

    /**
     * Mieter
     * 
     */
    @XmlEnumValue("103")
    MIETER("103"),

    /**
     * Garagen, Garage
     * 
     */
    @XmlEnumValue("104")
    GARAGEN("104"),

    /**
     * Pacht-/Mietwert
     * 
     */
    @XmlEnumValue("105")
    PACHT_MIETWERT("105"),

    /**
     * Aerzte, Arzt
     * 
     */
    @XmlEnumValue("107")
    AERZTE("107"),

    /**
     * Besamungen, Besamung
     * 
     */
    @XmlEnumValue("108")
    BESAMUNGEN("108"),

    /**
     * qm Stallungen, qm Stallung
     * 
     */
    @XmlEnumValue("109")
    QM_STALLUNGEN("109"),

    /**
     * Lehrer
     * 
     */
    @XmlEnumValue("110")
    LEHRER("110"),

    /**
     * Schueler
     * 
     */
    @XmlEnumValue("111")
    SCHUELER("111"),

    /**
     * Jagdscheine, Jagdschein
     * 
     */
    @XmlEnumValue("112")
    JAGDSCHEINE("112"),

    /**
     * km Strassenlaenge
     * 
     */
    @XmlEnumValue("113")
    KM_STRASSENLAENGE("113"),

    /**
     * Einwohner
     * 
     */
    @XmlEnumValue("115")
    EINWOHNER("115"),

    /**
     * Schiffe, Schiff
     * 
     */
    @XmlEnumValue("116")
    SCHIFFE("116"),

    /**
     * Schlepper
     * 
     */
    @XmlEnumValue("117")
    SCHLEPPER("117"),

    /**
     * PS
     * 
     */
    @XmlEnumValue("118")
    PS("118"),

    /**
     * BRT
     * 
     */
    @XmlEnumValue("120")
    BRT("120"),

    /**
     * kw
     * 
     */
    @XmlEnumValue("121")
    KW("121"),

    /**
     * Versicherungen, Versicherung
     * 
     */
    @XmlEnumValue("122")
    VERSICHERUNGEN("122"),

    /**
     * Warenumsatz
     * 
     */
    @XmlEnumValue("123")
    WARENUMSATZ("123"),

    /**
     * Betriebsorte, Betriebsort
     * 
     */
    @XmlEnumValue("124")
    BETRIEBSORTE("124"),

    /**
     * Staelle, Stall
     * 
     */
    @XmlEnumValue("126")
    STAELLE("126"),

    /**
     * GJ/H
     * 
     */
    @XmlEnumValue("127")
    GJ_H("127"),

    /**
     * Hilfspersonen, Hilfsperson
     * 
     */
    @XmlEnumValue("128")
    HILFSPERSONEN("128"),

    /**
     * Maschinen, Maschine
     * 
     */
    @XmlEnumValue("129")
    MASCHINEN("129"),

    /**
     * Behälter
     * 
     */
    @XmlEnumValue("800")
    BEHAELTER("800"),

    /**
     * Mitwirkende
     * 
     */
    @XmlEnumValue("801")
    MITWIRKENDE("801"),

    /**
     * Stück bzw. Wagnis / Wagnisse
     * 
     */
    @XmlEnumValue("802")
    STUECK_BZW_WAGNIS("802"),

    /**
     * Wert der Eigenleistung
     * 
     */
    @XmlEnumValue("803")
    WERT_DER_EIGENLEISTUNG("803"),

    /**
     * Vogel / Vögel
     * 
     */
    @XmlEnumValue("804")
    VOEGEL("804"),

    /**
     * Personen mit Arbeiten auf freiem Grundstück
     * 
     */
    @XmlEnumValue("805")
    PERSONEN_MIT_ARBEITEN_AUF_FREIEM_GRUNDSTUECK("805"),

    /**
     * Personen ohne Arbeiten auf freiem Grundstück
     * 
     */
    @XmlEnumValue("806")
    PERSONEN_OHNE_ARBEITEN_AUF_FREIEM_GRUNDSTUECK("806"),

    /**
     * Inhaber
     * 
     */
    @XmlEnumValue("807")
    INHABER("807"),

    /**
     * Angestellte
     * 
     */
    @XmlEnumValue("808")
    ANGESTELLTE("808"),

    /**
     * Stell- und Liegeplätze
     * 
     */
    @XmlEnumValue("809")
    STELL_LIEGEPLAETZE("809"),

    /**
     * Investitionssumme
     * 
     */
    @XmlEnumValue("810")
    INVESTITIONSSUMME("810"),

    /**
     * Wohn-/Geschäftseinheiten
     * 
     */
    @XmlEnumValue("811")
    WOHN_GESCHAEFTSEINHEITEN("811"),

    /**
     * Zählwerke
     * 
     */
    @XmlEnumValue("812")
    ZAEHLWERKE("812"),

    /**
     * Garderobenhaken
     * 
     */
    @XmlEnumValue("813")
    GARDEROBENHAKEN("813"),

    /**
     * Veranstaltungen
     * 
     */
    @XmlEnumValue("814")
    VERANSTALTUNGEN("814"),

    /**
     * Zelte
     * 
     */
    @XmlEnumValue("815")
    ZELTE("815"),

    /**
     * Kehrbezirke
     * 
     */
    @XmlEnumValue("816")
    KEHRBEZIRKE("816"),

    /**
     * Schiffsmaß in BRZ (Bruttoraumzahl)
     * 
     */
    @XmlEnumValue("817")
    SCHIFFSMASS_IN_BRZ("817"),

    /**
     * Behandlungsfälle
     * 
     */
    @XmlEnumValue("818")
    BEHANDLUNGSFAELLE("818"),

    /**
     * Sonstige
     * 
     */
    @XmlEnumValue("999")
    SONSTIGE("999");
    private final String value;

    STBerechnungseinheitHaftpflicht(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static STBerechnungseinheitHaftpflicht fromValue(String v) {
        for (STBerechnungseinheitHaftpflicht c: STBerechnungseinheitHaftpflicht.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
