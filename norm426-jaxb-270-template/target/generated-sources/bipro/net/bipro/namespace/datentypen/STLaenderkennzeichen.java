
package net.bipro.namespace.datentypen;

import javax.annotation.Generated;
import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for ST_Laenderkennzeichen.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="ST_Laenderkennzeichen"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="997"/&gt;
 *     &lt;enumeration value="998"/&gt;
 *     &lt;enumeration value="999"/&gt;
 *     &lt;enumeration value="A"/&gt;
 *     &lt;enumeration value="AFG"/&gt;
 *     &lt;enumeration value="AL"/&gt;
 *     &lt;enumeration value="AND"/&gt;
 *     &lt;enumeration value="AM"/&gt;
 *     &lt;enumeration value="AUS"/&gt;
 *     &lt;enumeration value="AZ"/&gt;
 *     &lt;enumeration value="B"/&gt;
 *     &lt;enumeration value="BD"/&gt;
 *     &lt;enumeration value="BDS"/&gt;
 *     &lt;enumeration value="BF"/&gt;
 *     &lt;enumeration value="BG"/&gt;
 *     &lt;enumeration value="BIH"/&gt;
 *     &lt;enumeration value="DY"/&gt;
 *     &lt;enumeration value="BOL"/&gt;
 *     &lt;enumeration value="BR"/&gt;
 *     &lt;enumeration value="BRN"/&gt;
 *     &lt;enumeration value="BRU"/&gt;
 *     &lt;enumeration value="BS"/&gt;
 *     &lt;enumeration value="BY"/&gt;
 *     &lt;enumeration value="BH"/&gt;
 *     &lt;enumeration value="BW"/&gt;
 *     &lt;enumeration value="BVI"/&gt;
 *     &lt;enumeration value="CU"/&gt;
 *     &lt;enumeration value="CAM"/&gt;
 *     &lt;enumeration value="CDN"/&gt;
 *     &lt;enumeration value="ZRE"/&gt;
 *     &lt;enumeration value="CH"/&gt;
 *     &lt;enumeration value="CI"/&gt;
 *     &lt;enumeration value="CL"/&gt;
 *     &lt;enumeration value="CO"/&gt;
 *     &lt;enumeration value="CR"/&gt;
 *     &lt;enumeration value="CY"/&gt;
 *     &lt;enumeration value="CZ"/&gt;
 *     &lt;enumeration value="D"/&gt;
 *     &lt;enumeration value="DK"/&gt;
 *     &lt;enumeration value="DOM"/&gt;
 *     &lt;enumeration value="DZ"/&gt;
 *     &lt;enumeration value="E"/&gt;
 *     &lt;enumeration value="EAK"/&gt;
 *     &lt;enumeration value="EAT"/&gt;
 *     &lt;enumeration value="EAZ"/&gt;
 *     &lt;enumeration value="EAU"/&gt;
 *     &lt;enumeration value="EC"/&gt;
 *     &lt;enumeration value="ER"/&gt;
 *     &lt;enumeration value="ES"/&gt;
 *     &lt;enumeration value="EST"/&gt;
 *     &lt;enumeration value="ET"/&gt;
 *     &lt;enumeration value="ETH"/&gt;
 *     &lt;enumeration value="F"/&gt;
 *     &lt;enumeration value="FIN"/&gt;
 *     &lt;enumeration value="FJI"/&gt;
 *     &lt;enumeration value="FL"/&gt;
 *     &lt;enumeration value="FR"/&gt;
 *     &lt;enumeration value="G"/&gt;
 *     &lt;enumeration value="GB"/&gt;
 *     &lt;enumeration value="GBA"/&gt;
 *     &lt;enumeration value="GBG"/&gt;
 *     &lt;enumeration value="GBJ"/&gt;
 *     &lt;enumeration value="GBM"/&gt;
 *     &lt;enumeration value="GBZ"/&gt;
 *     &lt;enumeration value="GCA"/&gt;
 *     &lt;enumeration value="GE"/&gt;
 *     &lt;enumeration value="GH"/&gt;
 *     &lt;enumeration value="GR"/&gt;
 *     &lt;enumeration value="GUY"/&gt;
 *     &lt;enumeration value="H"/&gt;
 *     &lt;enumeration value="HR"/&gt;
 *     &lt;enumeration value="HKJ"/&gt;
 *     &lt;enumeration value="HN"/&gt;
 *     &lt;enumeration value="I"/&gt;
 *     &lt;enumeration value="IL"/&gt;
 *     &lt;enumeration value="IND"/&gt;
 *     &lt;enumeration value="IR"/&gt;
 *     &lt;enumeration value="IRL"/&gt;
 *     &lt;enumeration value="IRQ"/&gt;
 *     &lt;enumeration value="IS"/&gt;
 *     &lt;enumeration value="J"/&gt;
 *     &lt;enumeration value="JA"/&gt;
 *     &lt;enumeration value="K"/&gt;
 *     &lt;enumeration value="KP"/&gt;
 *     &lt;enumeration value="KS"/&gt;
 *     &lt;enumeration value="KWT"/&gt;
 *     &lt;enumeration value="KZ"/&gt;
 *     &lt;enumeration value="L"/&gt;
 *     &lt;enumeration value="LAO"/&gt;
 *     &lt;enumeration value="LAR"/&gt;
 *     &lt;enumeration value="LB"/&gt;
 *     &lt;enumeration value="LS"/&gt;
 *     &lt;enumeration value="LT"/&gt;
 *     &lt;enumeration value="LV"/&gt;
 *     &lt;enumeration value="M"/&gt;
 *     &lt;enumeration value="MA"/&gt;
 *     &lt;enumeration value="MAL"/&gt;
 *     &lt;enumeration value="MC"/&gt;
 *     &lt;enumeration value="MD"/&gt;
 *     &lt;enumeration value="MEX"/&gt;
 *     &lt;enumeration value="MGL"/&gt;
 *     &lt;enumeration value="MK"/&gt;
 *     &lt;enumeration value="MNE"/&gt;
 *     &lt;enumeration value="MOC"/&gt;
 *     &lt;enumeration value="MS"/&gt;
 *     &lt;enumeration value="MW"/&gt;
 *     &lt;enumeration value="BUR"/&gt;
 *     &lt;enumeration value="N"/&gt;
 *     &lt;enumeration value="NA"/&gt;
 *     &lt;enumeration value="NAM"/&gt;
 *     &lt;enumeration value="NAU"/&gt;
 *     &lt;enumeration value="NEP"/&gt;
 *     &lt;enumeration value="WAN"/&gt;
 *     &lt;enumeration value="NIC"/&gt;
 *     &lt;enumeration value="NL"/&gt;
 *     &lt;enumeration value="NZ"/&gt;
 *     &lt;enumeration value="P"/&gt;
 *     &lt;enumeration value="PA"/&gt;
 *     &lt;enumeration value="PE"/&gt;
 *     &lt;enumeration value="PK"/&gt;
 *     &lt;enumeration value="PL"/&gt;
 *     &lt;enumeration value="PNG"/&gt;
 *     &lt;enumeration value="PY"/&gt;
 *     &lt;enumeration value="Q"/&gt;
 *     &lt;enumeration value="RA"/&gt;
 *     &lt;enumeration value="RC"/&gt;
 *     &lt;enumeration value="RCA"/&gt;
 *     &lt;enumeration value="RCB"/&gt;
 *     &lt;enumeration value="RCH"/&gt;
 *     &lt;enumeration value="RG"/&gt;
 *     &lt;enumeration value="RH"/&gt;
 *     &lt;enumeration value="RI"/&gt;
 *     &lt;enumeration value="RIM"/&gt;
 *     &lt;enumeration value="RL"/&gt;
 *     &lt;enumeration value="RM"/&gt;
 *     &lt;enumeration value="RMM"/&gt;
 *     &lt;enumeration value="RN"/&gt;
 *     &lt;enumeration value="ROK"/&gt;
 *     &lt;enumeration value="RO"/&gt;
 *     &lt;enumeration value="ROU"/&gt;
 *     &lt;enumeration value="RP"/&gt;
 *     &lt;enumeration value="RSM"/&gt;
 *     &lt;enumeration value="RU"/&gt;
 *     &lt;enumeration value="RUS"/&gt;
 *     &lt;enumeration value="RWA"/&gt;
 *     &lt;enumeration value="S"/&gt;
 *     &lt;enumeration value="SA"/&gt;
 *     &lt;enumeration value="SD"/&gt;
 *     &lt;enumeration value="SGP"/&gt;
 *     &lt;enumeration value="SK"/&gt;
 *     &lt;enumeration value="SLO"/&gt;
 *     &lt;enumeration value="SME"/&gt;
 *     &lt;enumeration value="SN"/&gt;
 *     &lt;enumeration value="SO"/&gt;
 *     &lt;enumeration value="SRB"/&gt;
 *     &lt;enumeration value="SUD"/&gt;
 *     &lt;enumeration value="SY"/&gt;
 *     &lt;enumeration value="SYR"/&gt;
 *     &lt;enumeration value="T"/&gt;
 *     &lt;enumeration value="TD"/&gt;
 *     &lt;enumeration value="TG"/&gt;
 *     &lt;enumeration value="TJ"/&gt;
 *     &lt;enumeration value="TM"/&gt;
 *     &lt;enumeration value="TN"/&gt;
 *     &lt;enumeration value="TR"/&gt;
 *     &lt;enumeration value="TT"/&gt;
 *     &lt;enumeration value="UA"/&gt;
 *     &lt;enumeration value="UAE"/&gt;
 *     &lt;enumeration value="USA"/&gt;
 *     &lt;enumeration value="UZ"/&gt;
 *     &lt;enumeration value="V"/&gt;
 *     &lt;enumeration value="VN"/&gt;
 *     &lt;enumeration value="WAG"/&gt;
 *     &lt;enumeration value="WAL"/&gt;
 *     &lt;enumeration value="WD"/&gt;
 *     &lt;enumeration value="WG"/&gt;
 *     &lt;enumeration value="WL"/&gt;
 *     &lt;enumeration value="WS"/&gt;
 *     &lt;enumeration value="WV"/&gt;
 *     &lt;enumeration value="YAR"/&gt;
 *     &lt;enumeration value="YV"/&gt;
 *     &lt;enumeration value="RNR"/&gt;
 *     &lt;enumeration value="ZA"/&gt;
 *     &lt;enumeration value="ZW"/&gt;
 *     &lt;enumeration value="SU"/&gt;
 *     &lt;enumeration value="YU"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(name = "ST_Laenderkennzeichen")
@XmlEnum
@Generated(value = "com.sun.tools.xjc.Driver", date = "2021-04-19T05:38:04+02:00", comments = "JAXB RI v2.3.2")
public enum STLaenderkennzeichen {


    /**
     * Mehrstaaten
     * 
     */
    @XmlEnumValue("997")
    MEHRSTAATEN("997"),

    /**
     * Sonstige
     * 
     */
    @XmlEnumValue("998")
    SONSTIGE("998"),

    /**
     * unbekannt
     * 
     */
    @XmlEnumValue("999")
    UNBEKANNT("999"),

    /**
     * Österreich
     * 
     */
    @XmlEnumValue("A")
    OESTERREICH("A"),

    /**
     * Afghanistan
     * 
     */
    @XmlEnumValue("AFG")
    AFGHANISTAN("AFG"),

    /**
     * Albanien
     * 
     */
    @XmlEnumValue("AL")
    ALBANIEN("AL"),

    /**
     * Andorra
     * 
     */
    @XmlEnumValue("AND")
    ANDORRA("AND"),

    /**
     * Armenien
     * 
     */
    @XmlEnumValue("AM")
    ARMENIEN("AM"),

    /**
     * Australien
     * 
     */
    @XmlEnumValue("AUS")
    AUSTRALIEN("AUS"),

    /**
     * Aserbaidschan
     * 
     */
    @XmlEnumValue("AZ")
    ASERBAIDSCHAN("AZ"),

    /**
     * Belgien
     * 
     */
    @XmlEnumValue("B")
    BELGIEN("B"),

    /**
     * Bangladesch
     * 
     */
    @XmlEnumValue("BD")
    BANGLADESCH("BD"),

    /**
     * Barbados
     * 
     */
    @XmlEnumValue("BDS")
    BARBADOS("BDS"),

    /**
     * Burkina Faso
     * 
     */
    @XmlEnumValue("BF")
    BURKINA_FASO("BF"),

    /**
     * Bulgarien
     * 
     */
    @XmlEnumValue("BG")
    BULGARIEN("BG"),

    /**
     * Bosnien und Herzegowina
     * 
     */
    @XmlEnumValue("BIH")
    BOSNIEN_UND_HERZEGOWINA("BIH"),

    /**
     * Benin
     * 
     */
    @XmlEnumValue("DY")
    BENIN("DY"),

    /**
     * Bolivien
     * 
     */
    @XmlEnumValue("BOL")
    BOLIVIEN("BOL"),

    /**
     * Brasilien
     * 
     */
    @XmlEnumValue("BR")
    BRASILIEN("BR"),

    /**
     * Bahrain
     * 
     */
    @XmlEnumValue("BRN")
    BAHRAIN("BRN"),

    /**
     * Brunei
     * 
     */
    @XmlEnumValue("BRU")
    BRUNEI("BRU"),

    /**
     * Bahamas
     * 
     */
    @XmlEnumValue("BS")
    BAHAMAS("BS"),

    /**
     * Weißrussland
     * 
     */
    @XmlEnumValue("BY")
    WEISSRUSSLAND("BY"),

    /**
     * Belize
     * 
     */
    @XmlEnumValue("BH")
    BELIZE("BH"),

    /**
     * Botswana
     * 
     */
    @XmlEnumValue("BW")
    BOTSWANA("BW"),

    /**
     * Jungferninseln
     * 
     */
    @XmlEnumValue("BVI")
    JUNGFERNINSELN("BVI"),

    /**
     * Kuba
     * 
     */
    @XmlEnumValue("CU")
    KUBA("CU"),

    /**
     * Kamerun
     * 
     */
    @XmlEnumValue("CAM")
    KAMERUN("CAM"),

    /**
     * Kanada
     * 
     */
    @XmlEnumValue("CDN")
    KANADA("CDN"),

    /**
     * Demokratische Republik Kongo
     * 
     */
    @XmlEnumValue("ZRE")
    DEMOKRATISCHE_REP_KONGO("ZRE"),

    /**
     * Schweiz
     * 
     */
    @XmlEnumValue("CH")
    SCHWEIZ("CH"),

    /**
     * Elfenbeinküste
     * 
     */
    @XmlEnumValue("CI")
    ELFENBEINKUESTE("CI"),

    /**
     * Sri Lanka
     * 
     */
    @XmlEnumValue("CL")
    SRI_LANKA("CL"),

    /**
     * Kolumbien
     * 
     */
    @XmlEnumValue("CO")
    KOLUMBIEN("CO"),

    /**
     * Costa Rica
     * 
     */
    @XmlEnumValue("CR")
    COSTA_RICA("CR"),

    /**
     * Zypern
     * 
     */
    @XmlEnumValue("CY")
    ZYPERN("CY"),

    /**
     * Tschechien
     * 
     */
    @XmlEnumValue("CZ")
    TSCHECHIEN("CZ"),

    /**
     * Deutschland
     * 
     */
    @XmlEnumValue("D")
    DEUTSCHLAND("D"),

    /**
     * Dänemark
     * 
     */
    @XmlEnumValue("DK")
    DAENEMARK("DK"),

    /**
     * Dominikanische Republik
     * 
     */
    @XmlEnumValue("DOM")
    DOMINIKANISCHE_REP("DOM"),

    /**
     * Algerien
     * 
     */
    @XmlEnumValue("DZ")
    ALGERIEN("DZ"),

    /**
     * Spanien
     * 
     */
    @XmlEnumValue("E")
    SPANIEN("E"),

    /**
     * Kenia
     * 
     */
    @XmlEnumValue("EAK")
    KENIA("EAK"),

    /**
     * Tansania
     * 
     */
    @XmlEnumValue("EAT")
    TANSANIA("EAT"),

    /**
     * Sansibar
     * 
     */
    @XmlEnumValue("EAZ")
    SANSIBAR("EAZ"),

    /**
     * Uganda
     * 
     */
    @XmlEnumValue("EAU")
    UGANDA("EAU"),

    /**
     * Ecuador
     * 
     */
    @XmlEnumValue("EC")
    ECUADOR("EC"),

    /**
     * Eritrea
     * 
     */
    @XmlEnumValue("ER")
    ERITREA("ER"),

    /**
     * El Salvador
     * 
     */
    @XmlEnumValue("ES")
    EL_SALVADOR("ES"),

    /**
     * Estland
     * 
     */
    @XmlEnumValue("EST")
    ESTLAND("EST"),

    /**
     * Ägypten
     * 
     */
    @XmlEnumValue("ET")
    AEGYPTEN("ET"),

    /**
     * Äthiopien
     * 
     */
    @XmlEnumValue("ETH")
    AETHIOPIEN("ETH"),

    /**
     * Frankreich
     * 
     */
    @XmlEnumValue("F")
    FRANKREICH("F"),

    /**
     * Finnland
     * 
     */
    @XmlEnumValue("FIN")
    FINNLAND("FIN"),

    /**
     * Fidschi
     * 
     */
    @XmlEnumValue("FJI")
    FIDSCHI("FJI"),

    /**
     * Liechtenstein
     * 
     */
    @XmlEnumValue("FL")
    LICHTENSTEIN("FL"),

    /**
     * Färöer
     * 
     */
    @XmlEnumValue("FR")
    FAEROEER("FR"),

    /**
     * Gabun
     * 
     */
    @XmlEnumValue("G")
    GABUN("G"),

    /**
     * Vereinigtes Königreich
     * 
     */
    @XmlEnumValue("GB")
    VEREINIGTES_KOENIGREICH("GB"),

    /**
     * Alderney
     * 
     */
    @XmlEnumValue("GBA")
    ALDERNEY("GBA"),

    /**
     * Guernsey
     * 
     */
    @XmlEnumValue("GBG")
    GUERNSEY("GBG"),

    /**
     * Jersey
     * 
     */
    @XmlEnumValue("GBJ")
    JERSEY("GBJ"),

    /**
     * Insel Man
     * 
     */
    @XmlEnumValue("GBM")
    INSEL_MAN("GBM"),

    /**
     * Gibraltar
     * 
     */
    @XmlEnumValue("GBZ")
    GIBRALTAR("GBZ"),

    /**
     * Guatemala
     * 
     */
    @XmlEnumValue("GCA")
    GUATEMALA("GCA"),

    /**
     * Georgien
     * 
     */
    @XmlEnumValue("GE")
    GEORGIEN("GE"),

    /**
     * Ghana
     * 
     */
    @XmlEnumValue("GH")
    GHANA("GH"),

    /**
     * Griechenland
     * 
     */
    @XmlEnumValue("GR")
    GRIECHENLAND("GR"),

    /**
     * Guyana
     * 
     */
    @XmlEnumValue("GUY")
    GUYANA("GUY"),

    /**
     * Ungarn
     * 
     */
    @XmlEnumValue("H")
    UNGARN("H"),

    /**
     * Kroatien
     * 
     */
    @XmlEnumValue("HR")
    KROATIEN("HR"),

    /**
     * Jordanien
     * 
     */
    @XmlEnumValue("HKJ")
    JORDANIEN("HKJ"),

    /**
     * Honduras
     * 
     */
    @XmlEnumValue("HN")
    HONDURAS("HN"),

    /**
     * Italien
     * 
     */
    @XmlEnumValue("I")
    ITALIEN("I"),

    /**
     * Israel
     * 
     */
    @XmlEnumValue("IL")
    ISRAEL("IL"),

    /**
     * Indien
     * 
     */
    @XmlEnumValue("IND")
    INDIEN("IND"),

    /**
     * Iran
     * 
     */
    @XmlEnumValue("IR")
    IRAN("IR"),

    /**
     * Irland
     * 
     */
    @XmlEnumValue("IRL")
    IRLAND("IRL"),

    /**
     * Irak
     * 
     */
    @XmlEnumValue("IRQ")
    IRAK("IRQ"),

    /**
     * Island
     * 
     */
    @XmlEnumValue("IS")
    ISLAND("IS"),

    /**
     * Japan
     * 
     */
    @XmlEnumValue("J")
    JAPAN("J"),

    /**
     * Jamaika
     * 
     */
    @XmlEnumValue("JA")
    JAMAIKA("JA"),

    /**
     * Kambodscha
     * 
     */
    @XmlEnumValue("K")
    KAMBODSCHA("K"),

    /**
     * Nordkorea
     * 
     */
    @XmlEnumValue("KP")
    NORDKOREA("KP"),

    /**
     * Kirgisistan
     * 
     */
    @XmlEnumValue("KS")
    KIRGISISTAN("KS"),

    /**
     * Kuwait
     * 
     */
    @XmlEnumValue("KWT")
    KUWAIT("KWT"),

    /**
     * Kasachstan
     * 
     */
    @XmlEnumValue("KZ")
    KASACHSTAN("KZ"),

    /**
     * Luxemburg
     * 
     */
    @XmlEnumValue("L")
    LUXEMBURG("L"),

    /**
     * Laos
     * 
     */
    @XmlEnumValue("LAO")
    LAOS("LAO"),

    /**
     * Libyen
     * 
     */
    @XmlEnumValue("LAR")
    LIBYEN("LAR"),

    /**
     * Liberia
     * 
     */
    @XmlEnumValue("LB")
    LIBERIA("LB"),

    /**
     * Lesotho
     * 
     */
    @XmlEnumValue("LS")
    LESOTHO("LS"),

    /**
     * Litauen
     * 
     */
    @XmlEnumValue("LT")
    LITAUEN("LT"),

    /**
     * Lettland
     * 
     */
    @XmlEnumValue("LV")
    LETTLAND("LV"),

    /**
     * Malta
     * 
     */
    @XmlEnumValue("M")
    MALTA("M"),

    /**
     * Marokko
     * 
     */
    @XmlEnumValue("MA")
    MAROKKO("MA"),

    /**
     * Malaysia
     * 
     */
    @XmlEnumValue("MAL")
    MALAYSIA("MAL"),

    /**
     * Monaco
     * 
     */
    @XmlEnumValue("MC")
    MONACO("MC"),

    /**
     * Moldawien
     * 
     */
    @XmlEnumValue("MD")
    MOLDAWIEN("MD"),

    /**
     * Mexiko
     * 
     */
    @XmlEnumValue("MEX")
    MEXIKO("MEX"),

    /**
     * Mongolei
     * 
     */
    @XmlEnumValue("MGL")
    MONGOLEI("MGL"),

    /**
     * Mazedonien
     * 
     */
    @XmlEnumValue("MK")
    MAZEDONIEN("MK"),

    /**
     * Montenegro
     * 
     */
    @XmlEnumValue("MNE")
    MONTENEGRO("MNE"),

    /**
     * Mosambik
     * 
     */
    @XmlEnumValue("MOC")
    MOSAMBIK("MOC"),

    /**
     * Mauritius
     * 
     */
    @XmlEnumValue("MS")
    MAURITIUS("MS"),

    /**
     * Malawi
     * 
     */
    @XmlEnumValue("MW")
    MALAWI("MW"),

    /**
     * Myanmar
     * 
     */
    @XmlEnumValue("BUR")
    MYANMAR("BUR"),

    /**
     * Norwegen
     * 
     */
    @XmlEnumValue("N")
    NORWEGEN("N"),

    /**
     * Niederländische Antillen
     * 
     */
    @XmlEnumValue("NA")
    NIEDERLAENDISCHE_ANTILLEN("NA"),

    /**
     * Namibia
     * 
     */
    @XmlEnumValue("NAM")
    NAMIBIA("NAM"),

    /**
     * Nauru
     * 
     */
    @XmlEnumValue("NAU")
    NAURU("NAU"),

    /**
     * Nepal
     * 
     */
    @XmlEnumValue("NEP")
    NEPAL("NEP"),

    /**
     * Nigeria
     * 
     */
    @XmlEnumValue("WAN")
    NIGERIA("WAN"),

    /**
     * Nicaragua
     * 
     */
    @XmlEnumValue("NIC")
    NICARAGUA("NIC"),

    /**
     * Niederlande
     * 
     */
    @XmlEnumValue("NL")
    NIEDERLANDE("NL"),

    /**
     * Neuseeland
     * 
     */
    @XmlEnumValue("NZ")
    NEUSEELAND("NZ"),

    /**
     * Portugal
     * 
     */
    @XmlEnumValue("P")
    PORTUGAL("P"),

    /**
     * Panama
     * 
     */
    @XmlEnumValue("PA")
    PANAMA("PA"),

    /**
     * Peru
     * 
     */
    @XmlEnumValue("PE")
    PERU("PE"),

    /**
     * Pakistan
     * 
     */
    @XmlEnumValue("PK")
    PAKISTAN("PK"),

    /**
     * Polen
     * 
     */
    @XmlEnumValue("PL")
    POLEN("PL"),

    /**
     * Papua-Neuguinea
     * 
     */
    @XmlEnumValue("PNG")
    PAPUA_NEUGUINEA("PNG"),

    /**
     * Paraguay
     * 
     */
    @XmlEnumValue("PY")
    PARAGUAY("PY"),

    /**
     * Katar
     * 
     */
    @XmlEnumValue("Q")
    KATAR("Q"),

    /**
     * Argentinien
     * 
     */
    @XmlEnumValue("RA")
    ARGENTINIEN("RA"),

    /**
     * Republik China (Taiwan)
     * 
     */
    @XmlEnumValue("RC")
    TAIWAN("RC"),

    /**
     * Zentralafrikanische Republik
     * 
     */
    @XmlEnumValue("RCA")
    ZENTRALAFRIKANISCHE_REP("RCA"),

    /**
     * Republik Kongo
     * 
     */
    @XmlEnumValue("RCB")
    REP_KONGO("RCB"),

    /**
     * Chile
     * 
     */
    @XmlEnumValue("RCH")
    CHILE("RCH"),

    /**
     * Guinea
     * 
     */
    @XmlEnumValue("RG")
    GUINEA("RG"),

    /**
     * Haiti
     * 
     */
    @XmlEnumValue("RH")
    HAITI("RH"),

    /**
     * Indonesien
     * 
     */
    @XmlEnumValue("RI")
    INDONESIEN("RI"),

    /**
     * Mauretanien
     * 
     */
    @XmlEnumValue("RIM")
    MAURETANIEN("RIM"),

    /**
     * Libanon
     * 
     */
    @XmlEnumValue("RL")
    LIBANON("RL"),

    /**
     * Madagaskar
     * 
     */
    @XmlEnumValue("RM")
    MADAGASKAR("RM"),

    /**
     * Mali
     * 
     */
    @XmlEnumValue("RMM")
    MALI("RMM"),

    /**
     * Niger
     * 
     */
    @XmlEnumValue("RN")
    NIGER("RN"),

    /**
     * Südkorea
     * 
     */
    @XmlEnumValue("ROK")
    SUEDKOREA("ROK"),

    /**
     * Rumänien
     * 
     */
    @XmlEnumValue("RO")
    RUMAENIEN("RO"),

    /**
     * Uruguay
     * 
     */
    @XmlEnumValue("ROU")
    URUGUAY("ROU"),

    /**
     * Philippinen
     * 
     */
    @XmlEnumValue("RP")
    PHILIPPINEN("RP"),

    /**
     * San Marino
     * 
     */
    @XmlEnumValue("RSM")
    SAN_MARINO("RSM"),

    /**
     * Burundi
     * 
     */
    @XmlEnumValue("RU")
    BURUNDI("RU"),

    /**
     * Russland
     * 
     */
    @XmlEnumValue("RUS")
    RUSSLAND("RUS"),

    /**
     * Ruanda
     * 
     */
    @XmlEnumValue("RWA")
    RUANDA("RWA"),

    /**
     * Schweden
     * 
     */
    @XmlEnumValue("S")
    SCHWEDEN("S"),

    /**
     * Saudi-Arabien
     * 
     */
    @XmlEnumValue("SA")
    SAUDI_ARABIEN("SA"),

    /**
     * Swasiland
     * 
     */
    @XmlEnumValue("SD")
    SWASILAND("SD"),

    /**
     * Singapur
     * 
     */
    @XmlEnumValue("SGP")
    SINGAPUR("SGP"),

    /**
     * Slowakei
     * 
     */
    @XmlEnumValue("SK")
    SLOWAKEI("SK"),

    /**
     * Slowenien
     * 
     */
    @XmlEnumValue("SLO")
    SLOWENIEN("SLO"),

    /**
     * Suriname
     * 
     */
    @XmlEnumValue("SME")
    SURINAME("SME"),

    /**
     * Senegal
     * 
     */
    @XmlEnumValue("SN")
    SENEGAL("SN"),

    /**
     * Somalia
     * 
     */
    @XmlEnumValue("SO")
    SOMALIA("SO"),

    /**
     * Serbien
     * 
     */
    @XmlEnumValue("SRB")
    SERBIEN("SRB"),

    /**
     * Sudan
     * 
     */
    @XmlEnumValue("SUD")
    SUDAN("SUD"),

    /**
     * Seychellen
     * 
     */
    @XmlEnumValue("SY")
    SEYCHELLEN("SY"),

    /**
     * Syrien
     * 
     */
    @XmlEnumValue("SYR")
    SYRIEN("SYR"),

    /**
     * Thailand
     * 
     */
    @XmlEnumValue("T")
    THAILAND("T"),

    /**
     * Tschad
     * 
     */
    @XmlEnumValue("TD")
    TSCHAD("TD"),

    /**
     * Togo
     * 
     */
    @XmlEnumValue("TG")
    TOGO("TG"),

    /**
     * Tadschikistan
     * 
     */
    @XmlEnumValue("TJ")
    TADSCHIKISTAN("TJ"),

    /**
     * Turkmenistan
     * 
     */
    @XmlEnumValue("TM")
    TURKMENISTAN("TM"),

    /**
     * Tunesien
     * 
     */
    @XmlEnumValue("TN")
    TUNESIEN("TN"),

    /**
     * Türkei
     * 
     */
    @XmlEnumValue("TR")
    TUERKEI("TR"),

    /**
     * Trinidad und Tobago
     * 
     */
    @XmlEnumValue("TT")
    TRINIDAD_UND_TOBAGO("TT"),

    /**
     * Ukraine
     * 
     */
    @XmlEnumValue("UA")
    UKRAINE("UA"),

    /**
     * Vereinigte Arabische Emirate
     * 
     */
    @XmlEnumValue("UAE")
    VEREINIGTE_ARABISCHE_EMIRATE("UAE"),

    /**
     * Vereinigte Staaten von Amerika
     * 
     */
    @XmlEnumValue("USA")
    VEREINIGTE_STAATEN_VON_AMERIKA("USA"),

    /**
     * Usbekistan
     * 
     */
    @XmlEnumValue("UZ")
    USBEKISTAN("UZ"),

    /**
     * Vatikanstaat
     * 
     */
    @XmlEnumValue("V")
    VATIKANSTAAT("V"),

    /**
     * Vietnam
     * 
     */
    @XmlEnumValue("VN")
    VIETNAM("VN"),

    /**
     * Gambia
     * 
     */
    @XmlEnumValue("WAG")
    GAMBIA("WAG"),

    /**
     * Sierra Leone
     * 
     */
    @XmlEnumValue("WAL")
    SIERRA_LEONE("WAL"),

    /**
     * Dominica
     * 
     */
    @XmlEnumValue("WD")
    DOMINICA("WD"),

    /**
     * Grenada
     * 
     */
    @XmlEnumValue("WG")
    GRENADA("WG"),

    /**
     * St. Lucia
     * 
     */
    @XmlEnumValue("WL")
    ST_LUCIA("WL"),

    /**
     * Samoa
     * 
     */
    @XmlEnumValue("WS")
    SAMOA("WS"),

    /**
     * St. Vincent und die Grenadinen
     * 
     */
    @XmlEnumValue("WV")
    ST_VINCENT_UND_DIE_GRENADINEN("WV"),

    /**
     * Jemen
     * 
     */
    @XmlEnumValue("YAR")
    JEMEN("YAR"),

    /**
     * Venezuela
     * 
     */
    @XmlEnumValue("YV")
    VENEZUELA("YV"),

    /**
     * Sambia
     * 
     */
    @XmlEnumValue("RNR")
    SAMBIA("RNR"),

    /**
     * Südafrika
     * 
     */
    @XmlEnumValue("ZA")
    SUEDAFRIKA("ZA"),

    /**
     * Simbabwe
     * 
     */
    @XmlEnumValue("ZW")
    SIMBABWE("ZW"),

    /**
     * Sowjetunion
     * 
     */
    @XmlEnumValue("SU")
    SOWJETUNION("SU"),

    /**
     * Jugoslawien
     * 
     */
    @XmlEnumValue("YU")
    JUGOSLAWIEN("YU");
    private final String value;

    STLaenderkennzeichen(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static STLaenderkennzeichen fromValue(String v) {
        for (STLaenderkennzeichen c: STLaenderkennzeichen.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
