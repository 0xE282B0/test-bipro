
package net.bipro.namespace.datentypen;

import javax.annotation.Generated;
import javax.xml.bind.annotation.XmlEnum;
import javax.xml.bind.annotation.XmlEnumValue;
import javax.xml.bind.annotation.XmlType;


/**
 * <p>Java class for ST_Sparte.
 * 
 * <p>The following schema fragment specifies the expected content contained within this class.
 * <p>
 * <pre>
 * &lt;simpleType name="ST_Sparte"&gt;
 *   &lt;restriction base="{http://www.w3.org/2001/XMLSchema}string"&gt;
 *     &lt;enumeration value="010"/&gt;
 *     &lt;enumeration value="020"/&gt;
 *     &lt;enumeration value="030"/&gt;
 *     &lt;enumeration value="040"/&gt;
 *     &lt;enumeration value="050"/&gt;
 *     &lt;enumeration value="051"/&gt;
 *     &lt;enumeration value="052"/&gt;
 *     &lt;enumeration value="053"/&gt;
 *     &lt;enumeration value="054"/&gt;
 *     &lt;enumeration value="055"/&gt;
 *     &lt;enumeration value="060"/&gt;
 *     &lt;enumeration value="070"/&gt;
 *     &lt;enumeration value="080"/&gt;
 *     &lt;enumeration value="081"/&gt;
 *     &lt;enumeration value="082"/&gt;
 *     &lt;enumeration value="083"/&gt;
 *     &lt;enumeration value="089"/&gt;
 *     &lt;enumeration value="090"/&gt;
 *     &lt;enumeration value="099"/&gt;
 *     &lt;enumeration value="100"/&gt;
 *     &lt;enumeration value="109"/&gt;
 *     &lt;enumeration value="110"/&gt;
 *     &lt;enumeration value="015"/&gt;
 *     &lt;enumeration value="120"/&gt;
 *     &lt;enumeration value="130"/&gt;
 *     &lt;enumeration value="140"/&gt;
 *     &lt;enumeration value="150"/&gt;
 *     &lt;enumeration value="160"/&gt;
 *     &lt;enumeration value="171"/&gt;
 *     &lt;enumeration value="172"/&gt;
 *     &lt;enumeration value="174"/&gt;
 *     &lt;enumeration value="175"/&gt;
 *     &lt;enumeration value="176"/&gt;
 *     &lt;enumeration value="179"/&gt;
 *     &lt;enumeration value="180"/&gt;
 *     &lt;enumeration value="181"/&gt;
 *     &lt;enumeration value="210"/&gt;
 *     &lt;enumeration value="231"/&gt;
 *     &lt;enumeration value="232"/&gt;
 *     &lt;enumeration value="233"/&gt;
 *     &lt;enumeration value="290"/&gt;
 *     &lt;enumeration value="293"/&gt;
 *     &lt;enumeration value="294"/&gt;
 *     &lt;enumeration value="299"/&gt;
 *     &lt;enumeration value="510"/&gt;
 *     &lt;enumeration value="550"/&gt;
 *     &lt;enumeration value="580"/&gt;
 *     &lt;enumeration value="600"/&gt;
 *     &lt;enumeration value="630"/&gt;
 *     &lt;enumeration value="650"/&gt;
 *     &lt;enumeration value="011"/&gt;
 *     &lt;enumeration value="012"/&gt;
 *     &lt;enumeration value="013"/&gt;
 *     &lt;enumeration value="014"/&gt;
 *     &lt;enumeration value="021"/&gt;
 *     &lt;enumeration value="022"/&gt;
 *     &lt;enumeration value="023"/&gt;
 *     &lt;enumeration value="024"/&gt;
 *     &lt;enumeration value="025"/&gt;
 *     &lt;enumeration value="026"/&gt;
 *     &lt;enumeration value="027"/&gt;
 *     &lt;enumeration value="028"/&gt;
 *     &lt;enumeration value="029"/&gt;
 *     &lt;enumeration value="031"/&gt;
 *     &lt;enumeration value="033"/&gt;
 *     &lt;enumeration value="034"/&gt;
 *     &lt;enumeration value="038"/&gt;
 *     &lt;enumeration value="039"/&gt;
 *     &lt;enumeration value="041"/&gt;
 *     &lt;enumeration value="042"/&gt;
 *     &lt;enumeration value="043"/&gt;
 *     &lt;enumeration value="044"/&gt;
 *     &lt;enumeration value="045"/&gt;
 *     &lt;enumeration value="046"/&gt;
 *     &lt;enumeration value="047"/&gt;
 *     &lt;enumeration value="048"/&gt;
 *     &lt;enumeration value="049"/&gt;
 *     &lt;enumeration value="059"/&gt;
 *     &lt;enumeration value="063"/&gt;
 *     &lt;enumeration value="065"/&gt;
 *     &lt;enumeration value="069"/&gt;
 *     &lt;enumeration value="071"/&gt;
 *     &lt;enumeration value="072"/&gt;
 *     &lt;enumeration value="073"/&gt;
 *     &lt;enumeration value="075"/&gt;
 *     &lt;enumeration value="076"/&gt;
 *     &lt;enumeration value="079"/&gt;
 *     &lt;enumeration value="123"/&gt;
 *     &lt;enumeration value="124"/&gt;
 *     &lt;enumeration value="131"/&gt;
 *     &lt;enumeration value="132"/&gt;
 *     &lt;enumeration value="141"/&gt;
 *     &lt;enumeration value="142"/&gt;
 *     &lt;enumeration value="161"/&gt;
 *     &lt;enumeration value="162"/&gt;
 *     &lt;enumeration value="169"/&gt;
 *     &lt;enumeration value="170"/&gt;
 *     &lt;enumeration value="182"/&gt;
 *     &lt;enumeration value="183"/&gt;
 *     &lt;enumeration value="184"/&gt;
 *     &lt;enumeration value="185"/&gt;
 *     &lt;enumeration value="189"/&gt;
 *     &lt;enumeration value="190"/&gt;
 *     &lt;enumeration value="191"/&gt;
 *     &lt;enumeration value="192"/&gt;
 *     &lt;enumeration value="193"/&gt;
 *     &lt;enumeration value="194"/&gt;
 *     &lt;enumeration value="197"/&gt;
 *     &lt;enumeration value="199"/&gt;
 *     &lt;enumeration value="200"/&gt;
 *     &lt;enumeration value="201"/&gt;
 *     &lt;enumeration value="202"/&gt;
 *     &lt;enumeration value="209"/&gt;
 *     &lt;enumeration value="230"/&gt;
 *     &lt;enumeration value="240"/&gt;
 *     &lt;enumeration value="241"/&gt;
 *     &lt;enumeration value="242"/&gt;
 *     &lt;enumeration value="243"/&gt;
 *     &lt;enumeration value="244"/&gt;
 *     &lt;enumeration value="249"/&gt;
 *     &lt;enumeration value="250"/&gt;
 *     &lt;enumeration value="251"/&gt;
 *     &lt;enumeration value="252"/&gt;
 *     &lt;enumeration value="291"/&gt;
 *     &lt;enumeration value="296"/&gt;
 *     &lt;enumeration value="300"/&gt;
 *     &lt;enumeration value="560"/&gt;
 *     &lt;enumeration value="570"/&gt;
 *     &lt;enumeration value="684"/&gt;
 *     &lt;enumeration value="590"/&gt;
 *   &lt;/restriction&gt;
 * &lt;/simpleType&gt;
 * </pre>
 * 
 */
@XmlType(name = "ST_Sparte")
@XmlEnum
@Generated(value = "com.sun.tools.xjc.Driver", date = "2021-04-19T05:38:04+02:00", comments = "JAXB RI v2.3.2")
public enum STSparte {


    /**
     * Leben
     * 
     */
    @XmlEnumValue("010")
    LEBEN("010"),

    /**
     * Krankenversicherung
     * 
     */
    @XmlEnumValue("020")
    KRANKENVERSICHERUNG("020"),

    /**
     * Allgemeine Unfallversicherung
     * 
     */
    @XmlEnumValue("030")
    ALLGEMEINE_UNFALLVERSICHERUNG("030"),

    /**
     * Haftpflicht
     * 
     */
    @XmlEnumValue("040")
    HAFTPFLICHT("040"),

    /**
     * Kraftfahrt
     * 
     */
    @XmlEnumValue("050")
    KRAFTFAHRT("050"),

    /**
     * Kfz-Haftpflicht
     * 
     */
    @XmlEnumValue("051")
    KFZ_HAFTPFLICHT("051"),

    /**
     * Kfz-Fahrzeugvoll
     * 
     */
    @XmlEnumValue("052")
    KFZ_FAHRZEUGVOLL("052"),

    /**
     * Kfz-Fahrzeugteil
     * 
     */
    @XmlEnumValue("053")
    KFZ_FAHRZEUGTEIL("053"),

    /**
     * Kfz-Unfall
     * 
     */
    @XmlEnumValue("054")
    KFZ_UNFALL("054"),

    /**
     * Kfz-Baustein
     * 
     */
    @XmlEnumValue("055")
    KFZ_BAUSTEIN("055"),

    /**
     * Luftfahrtversicherung
     * 
     */
    @XmlEnumValue("060")
    LUFTFAHRTVERSICHERUNG("060"),

    /**
     * Rechtsschutz
     * 
     */
    @XmlEnumValue("070")
    RECHTSSCHUTZ("070"),

    /**
     * Feuerversicherung
     * 
     */
    @XmlEnumValue("080")
    FEUERVERSICHERUNG("080"),

    /**
     * Feuer-Industrie-Versicherung
     * 
     */
    @XmlEnumValue("081")
    FEUER_INDUSTRIE_VERSICHERUNG("081"),

    /**
     * landwirtschaftliche Feuer-Versicherung
     * 
     */
    @XmlEnumValue("082")
    LANDWIRTSCHAFTLICHE_FEUER_VERSICHERUNG("082"),

    /**
     * sonstige Feuerversicherung
     * 
     */
    @XmlEnumValue("083")
    SONSTIGE_FEUERVERSICHERUNG("083"),

    /**
     * uebrige und nicht aufgegliederte Feuer-Versicherung
     * 
     */
    @XmlEnumValue("089")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_FEUERVERSICHERUNG("089"),

    /**
     * Einbruch-Diebstahl- und Raub(ED)-Versicherung
     * 
     */
    @XmlEnumValue("090")
    EINBRUCH_DIEBSTAHL_RAUB_VERSICHERUNG("090"),

    /**
     * uebrige und nicht aufgegliederte ED-Versicherung
     * 
     */
    @XmlEnumValue("099")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_ED_VERSICHERUNG("099"),

    /**
     * Leitungswasser(LW)-Versicherung
     * 
     */
    @XmlEnumValue("100")
    LEITUNGSWASSER_VERSICHERUNG("100"),

    /**
     * uebrige und nicht aufgegliederte LW-Versicherung
     * 
     */
    @XmlEnumValue("109")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_LW_VERSICHERUNG("109"),

    /**
     * Glasversicherung
     * 
     */
    @XmlEnumValue("110")
    GLASVERSICHERUNG("110"),

    /**
     * Geschäfte der Verwaltung von Versorgungseinrichtungen
     * 
     */
    @XmlEnumValue("015")
    GESCHAEFTE_DER_VERWALTUNG_VON_VERSORGUNGSEINRICHTUNGEN("015"),

    /**
     * Sturm-Versicherung
     * 
     */
    @XmlEnumValue("120")
    STURM_VERSICHERUNG("120"),

    /**
     * Verbundene Hausrat
     * 
     */
    @XmlEnumValue("130")
    VERBUNDENE_HAUSRAT("130"),

    /**
     * Verbundene Gebaeude
     * 
     */
    @XmlEnumValue("140")
    VERBUNDENE_GEBAEUDE("140"),

    /**
     * Hagel-Versicherung
     * 
     */
    @XmlEnumValue("150")
    HAGEL_VERSICHERUNG("150"),

    /**
     * Tierversicherung
     * 
     */
    @XmlEnumValue("160")
    TIERVERSICHERUNG("160"),

    /**
     * Maschinen-Versicherung
     * 
     */
    @XmlEnumValue("171")
    MASCHINEN_VERSICHERUNG("171"),

    /**
     * Elektronik-Versicherung
     * 
     */
    @XmlEnumValue("172")
    ELEKTRONIK_VERSICHERUNG("172"),

    /**
     * Montage-Versicherung
     * 
     */
    @XmlEnumValue("174")
    MONTAGE_VERSICHERUNG("174"),

    /**
     * Haushaltsgeraeteversicherung
     * 
     */
    @XmlEnumValue("175")
    HAUSHALTSGERAETEVERSICHERUNG("175"),

    /**
     * Bauleistungs-Versicherung
     * 
     */
    @XmlEnumValue("176")
    BAULEISTUNGS_VERSICHERUNG("176"),

    /**
     * uebrige Technische Versicherungen
     * 
     */
    @XmlEnumValue("179")
    UEBRIGE_TECHNISCHE_VERSICHERUNGEN("179"),

    /**
     * Einheits-Versicherung
     * 
     */
    @XmlEnumValue("180")
    EINHEITS_VERSICHERUNG("180"),

    /**
     * Allgemeine Einheitsversicherung
     * 
     */
    @XmlEnumValue("181")
    ALLGEMEINE_EINHEITSVERSICHERUNG("181"),

    /**
     * Extend Coverage(EC)-Versicherung
     * 
     */
    @XmlEnumValue("210")
    EC_VERSICHERUNG("210"),

    /**
     * Feuer-(Gross)Betriebsunterbrechungs-Versicherung
     * 
     */
    @XmlEnumValue("231")
    FEUER_BETRIEBSUNTERBRECHUNGS_VERSICHERUNG("231"),

    /**
     * Technische-Betriebsunterbrechungs-Versicherung/ MBU Elektronik
     * 
     */
    @XmlEnumValue("232")
    TECHNISCHE_BETRIEBSUNTERBRECHUNGS_VERSICHERUNG("232"),

    /**
     * sonstige Betriebsunterbrechungsversicherungen
     * 
     */
    @XmlEnumValue("233")
    SONSTIGE_BETRIEBSUNTERBRECHUNGSVERSICHERUNGEN("233"),

    /**
     * sonstige Schadenversicherung, z. B. Maschinen-Garantie-Versicherung
     * 
     */
    @XmlEnumValue("290")
    SONSTIGE_SCHADENVERSICHERUNGEN("290"),

    /**
     * sonstige Vermoegensschadenversicherungen
     * 
     */
    @XmlEnumValue("293")
    SONSTIGE_VERMOEGENSSCHADENVERSICHERUNGEN("293"),

    /**
     * sonstige gemischte Versicherungen
     * 
     */
    @XmlEnumValue("294")
    SONSTIGE_GEMISCHTE_VERSICHERUNGEN("294"),

    /**
     * uebrige und nicht aufgegliederte sonstige Schadenversicherungen
     * 
     */
    @XmlEnumValue("299")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_SCHADENSVERSICHERUNGEN("299"),

    /**
     * Verkehrsservice
     * 
     */
    @XmlEnumValue("510")
    VERKEHRSSERVICE("510"),

    /**
     * Investmentfonds
     * 
     */
    @XmlEnumValue("550")
    INVESTMENTFONDS("550"),

    /**
     * Bausparkassen
     * 
     */
    @XmlEnumValue("580")
    BAUSPARKASSEN("580"),

    /**
     * Moped 2.
     * 
     */
    @XmlEnumValue("600")
    MOPED_2("600"),

    /**
     * Rahmen- und Sammelvertraege
     * 
     */
    @XmlEnumValue("630")
    RAHMEN_SAMMELVERTRAEGE("630"),

    /**
     * Handel und Handwerk 2.
     * 
     */
    @XmlEnumValue("650")
    HANDEL_HANDWERK_2("650"),

    /**
     * Einzelversicherung (ohne Zusatzversicherung) mit
     * Überschussbeteiligung, bei der das Anlagerisiko vom
     * Versicherungsunternehmen getragen wird.
     * 
     */
    @XmlEnumValue("011")
    EINZELVERSICHERUNG_UEBERSCHUSSBETEILIGUNG("011"),

    /**
     * Kollektivversicherung (ohne Zusatzversicherung) mit
     * Überschussbeteiligung, bei der das Anlagerisiko vom
     * Versicherungsunternehmen getragen wird.
     * 
     */
    @XmlEnumValue("012")
    KOLLEKTIVVERSICHERUNG_UEBERSCHUSSBETEILIGUNG("012"),

    /**
     * Zusatzversicherung (einschließlich der für
     * Kollektivversicherungen)
     * 
     */
    @XmlEnumValue("013")
    ZUSATZVERSICHERUNG("013"),

    /**
     * Sonstige Lebensversicherung
     * 
     */
    @XmlEnumValue("014")
    SONSTIGE_LEBENSVERSICHERUNG("014"),

    /**
     * Einzel- Krankheitskostenvollversicherung (ambulant und
     * stationär)
     * 
     */
    @XmlEnumValue("021")
    EINZEL_KRANKHEITSKOSTENVOLLVERSICHERUNG("021"),

    /**
     * selbstständige Einzel- Krankheitskostenversicherung
     * (ambulant)
     * 
     */
    @XmlEnumValue("022")
    SELBSTSTAENDIGE_EINZEL_KRANKHEITSKOSTENVERSICHERUNG_AMBULANT("022"),

    /**
     * selbstständige Einzel- Krankheitskostenversicherung
     * (stationär)
     * 
     */
    @XmlEnumValue("023")
    SELBSTSTAENDIGE_EINZEL_KRANKHEITSKOSTENVERSICHERUNG_STATIONAER("023"),

    /**
     * Einzel- Krankentagegeldversicherung
     * 
     */
    @XmlEnumValue("024")
    EINZEL_KRANKENTAGEGELDVERSICHERUNG("024"),

    /**
     * selbstständige Einzel- Krankenhaustagegeldversicherung
     * 
     */
    @XmlEnumValue("025")
    SELBSTSTAENDIGE_EINZEL_KRANKENHAUSTAGEGELDVERSICHERUNG("025"),

    /**
     * sonstige selbstständige Einzel- Teilversicherung
     * 
     */
    @XmlEnumValue("026")
    SONSTIGE_SELBSTSTAENDIGE_EINZEL_TEILVERSICHERUNG("026"),

    /**
     * Gruppen- Krankenversicherung (nach Einzel- und
     * Sondertarifen)
     * 
     */
    @XmlEnumValue("027")
    GRUPPEN_KRANKENVERSICHERUNG("027"),

    /**
     * Pflegekrankenversicherung
     * 
     */
    @XmlEnumValue("028")
    PFLEGEKRANKENVERSICHERUNG("028"),

    /**
     * übrige und nicht aufgegliederte Krankenversicherung
     * (einschließlich der Beihilfeablöseversicherung)
     * 
     */
    @XmlEnumValue("029")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_KRANKENVERSICHERUNG("029"),

    /**
     * Einzelunfallversicherung ohne Beitragsrückgewähr
     * 
     */
    @XmlEnumValue("031")
    EINZELUNFALLVERSICHERUNG_OHNE_BEITRAGSRUECKGEWAEHR("031"),

    /**
     * Gruppen- Unfallversicherung ohne Beitragsrückgewähr
     * 
     */
    @XmlEnumValue("033")
    GRUPPEN_UNFALLVERSICHERUNG_OHNE_BEITRAGSRUECKGEWAEHR("033"),

    /**
     * Probandenversicherung
     * 
     */
    @XmlEnumValue("034")
    PROBANDENVERSICHERUNG("034"),

    /**
     * Unfallversicherung mit Beitragsrückgewähr
     * 
     */
    @XmlEnumValue("038")
    UNFALLVERSICHERUNG_MIT_BEITRAGSRUECKGEWAEHR("038"),

    /**
     * übrige und nicht aufgegliederte Allgemeine
     * Unfallversicherung
     * 
     */
    @XmlEnumValue("039")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_ALLGEMEINE_UNFALLVERSICHERUNG("039"),

    /**
     * Privathaftpflichtversicherung (einschließlich Sportboot- und
     * Hundehalterhaftpflichtversicherung)
     * 
     */
    @XmlEnumValue("041")
    PRIVATHAFTPFLICHTVERSICHERUNG("041"),

    /**
     * Betriebs- und Berufshaftpflichtversicherung
     * 
     */
    @XmlEnumValue("042")
    BETRIEBS_BERUFSHAFTPFLICHTVERSICHERUNG("042"),

    /**
     * Umwelt- Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("043")
    UMWELT_HAFTPFLICHTVERSICHERUNG("043"),

    /**
     * Vermögensschaden- Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("044")
    VERMOEGENSSCHADEN_HAFTPFLICHTVERSICHERUNG("044"),

    /**
     * Verkehrshaftungsversicherung (einschließlich der Speditions- und Rollfuhrversicherung)
     * 
     */
    @XmlEnumValue("045")
    VERKEHRSHAFTUNGSVERSICHERUNG("045"),

    /**
     * Strahlen- und Atomanlagen-Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("046")
    STRAHLEN_ATOMANLAGEN_HAFTPFLICHTVERSICHERUNG("046"),

    /**
     * Feuerhaftungsversicherung
     * 
     */
    @XmlEnumValue("047")
    FEUERHAFTUNGSVERSICHERUNG("047"),

    /**
     * See-, Binnensee- und Flussschifffahrtshaftpflichtversicherung (ohne Kollisionshaftpflichtrisiko) sowie Haftpflichtversicherung für
     * nichtversicherungspflichtige Landfahrzeuge)
     * 
     */
    @XmlEnumValue("048")
    SEE_BINNENSEE_FLUSSSCHIFFFAHRTSHAFTPFLICHTVERSICHERUNG("048"),

    /**
     * übrige und nicht aufgegliederte Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("049")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_HAFTPFLICHTVERSICHERUNG("049"),

    /**
     * Kfz-Gepäck
     * 
     */
    @XmlEnumValue("059")
    KFZ_GEPAECK("059"),

    /**
     * Luftfahrzeug-Kaskoversicherung
     * 
     */
    @XmlEnumValue("063")
    LUFTFAHRZEUG_KASKOVERSICHERUNG("063"),

    /**
     * Raumfahrzeug-Kaskoversicherung
     * 
     */
    @XmlEnumValue("065")
    RAUMFAHRZEUG_KASKOVERSICHERUNG("065"),

    /**
     * übrige und nicht aufgegliederte Luftfahrtversicherung
     * (einschließlich der Raumfahrtversicherung)
     * 
     */
    @XmlEnumValue("069")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_LUFTFAHRTVERSICHERUNG("069"),

    /**
     * Rechtsschutzversicherung nach ARB
     * 
     */
    @XmlEnumValue("071")
    RECHTSSCHUTZVERSICHERUNG_ARB("071"),

    /**
     * Vermögensschaden-Rechtsschutzversicherung für
     * Aufsichtsräte, Beiräte, Vorstände (VRB)
     * 
     */
    @XmlEnumValue("072")
    VERMOEGENSSCHADEN_RECHTSSCHUTZVERSICHERUNG_AUFICHTSRAETE_BEIRAETE_VORSTAENDE("072"),

    /**
     * Rechtsschutzversicherung für Träger öffentlicher Aufgaben
     * (ÖRB)
     * 
     */
    @XmlEnumValue("073")
    RECHTSSCHUTZVERSICHERUNG_TRAEGER_OEFFENTLICHER_AUFGABEN("073"),

    /**
     * Kraftfahrt-Strafrechtsschutzversicherung mit
     * Auslands-Zivilrechtsschutzversicherung
     * 
     */
    @XmlEnumValue("075")
    KRAFTFAHRT_STRAFRECHTSSCHUTZVERSICHERUNG_MIT_AUSLANDS_ZIVILRECHTSSCHUTZVERSICHERUNG("075"),

    /**
     * Spezial-Strafrechtsschutzversicherung der Unternehmen
     * 
     */
    @XmlEnumValue("076")
    SPEZIAL_STRAFRECHTSSCHUTZVERSICHERUNG_UNTERNEHMEN("076"),

    /**
     * übrige und nicht aufgegliederte Rechtsschutzversicherung
     * 
     */
    @XmlEnumValue("079")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_RECHTSSCHUTZVERSICHERUNG("079"),

    /**
     * Gärtnerei-Sturmversicherung
     * 
     */
    @XmlEnumValue("123")
    GAERTNEREI_STURMVERSICHERUNG("123"),

    /**
     * Versicherung weiterer Elementarschäden bei gewerblichen
     * Risiken
     * 
     */
    @XmlEnumValue("124")
    VERSICHERUNG_ELEMENTARSCHAEDEN_GEWERBLICHE_RISIKEN("124"),

    /**
     * Verbundene Hausratversicherung ohne Einschluss weiterer
     * Elementarschäden
     * 
     */
    @XmlEnumValue("131")
    VERBUNDENE_HAUSRATVERSICHERUNG_OHNE_ELEMENTARSCHAEDEN("131"),

    /**
     * Verbundene Hausratversicherung unter Einschluss weiterer
     * Elementarschäden
     * 
     */
    @XmlEnumValue("132")
    VERBUNDENE_HAUSRATVERSICHERUNG_ELEMENTARSCHAEDEN("132"),

    /**
     * Verbundene Wohngebäudeversicherung ohne Einfluss
     * weiterer Elementarschäden
     * 
     */
    @XmlEnumValue("141")
    VERBUNDENE_WOHNGEBAEUDEVERSICHERUNG_OHNE_ELEMENTARSCHAEDEN("141"),

    /**
     * Verbundene Wohngebäudeversicherung unter Einfluss
     * weiterer Elementarschäden
     * 
     */
    @XmlEnumValue("142")
    VERBUNDENE_WOHNGEBAEUDEVERSICHERUNG_ELEMENTARSCHAEDEN("142"),

    /**
     * langfristige Tierlebensversicherung
     * 
     */
    @XmlEnumValue("161")
    LANGFRISTIGE_TIERLEBENSVERSICHERUNG("161"),

    /**
     * kurzfristige Tierversicherung
     * 
     */
    @XmlEnumValue("162")
    KURZFRISTIGE_TIERVERSICHERUNG("162"),

    /**
     * übrige und nicht aufgegliederte Tierversicherung
     * 
     */
    @XmlEnumValue("169")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_TIERVERSICHERUNG("169"),

    /**
     * Technische Versicherung
     * 
     */
    @XmlEnumValue("170")
    TECHNISCHE_VERSICHERUNG("170"),

    /**
     * Juwelierwaren-Einheitsversicherung
     * 
     */
    @XmlEnumValue("182")
    JUWELIERWAREN_EINHEITSVERSICHERUNG("182"),

    /**
     * Rauchwaren-Einheitsversicherung
     * 
     */
    @XmlEnumValue("183")
    RAUCHWAREN_EINHEITSVERSICHERUNG("183"),

    /**
     * Textilveredlungs-Einheitsversicherung
     * 
     */
    @XmlEnumValue("184")
    TEXTILVEREDELUNGS_EINHEITSVERSICHERUNG("184"),

    /**
     * Wäscheschutz-Einheitsversicherung
     * 
     */
    @XmlEnumValue("185")
    WAESCHESCHUTZ_EINHEITSVERSICHERUNG("185"),

    /**
     * übrige und nicht aufgegliederte Einheitsversicherung
     * 
     */
    @XmlEnumValue("189")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_EINHEITSVERSICHERUNG("189"),

    /**
     * Transport
     * 
     */
    @XmlEnumValue("190")
    TRANSPORT("190"),

    /**
     * Kaskoversicherung
     * 
     */
    @XmlEnumValue("191")
    KASKOVERSICHERUNG("191"),

    /**
     * Transportgüterversicherung
     * 
     */
    @XmlEnumValue("192")
    TRANSPORTGUETERVERSICHERUNG("192"),

    /**
     * Valorenversicherung (gewerblich)
     * 
     */
    @XmlEnumValue("193")
    VALORENVERSICHERUNG("193"),

    /**
     * Filmversicherung
     * 
     */
    @XmlEnumValue("194")
    FILMVERSICHERUNG("194"),

    /**
     * Kriegsrisikoversicherung
     * 
     */
    @XmlEnumValue("197")
    KRIEGSRISIKOVERSICHERUNG("197"),

    /**
     * übrige und nicht aufgegliederte Transportversicherung
     * (einschließlich Versicherung von Offshore-Risiken)
     * 
     */
    @XmlEnumValue("199")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_TRANSPORTVERSICHERUNG("199"),

    /**
     * Kredit- und Kautionsversicherung
     * 
     */
    @XmlEnumValue("200")
    KREDIT_KAUTIONSVERSICHERUNG("200"),

    /**
     * Kautionsversicherung (einschließlich
     * Baugarantieversicherung)
     * 
     */
    @XmlEnumValue("201")
    KAUTIONSVERSICHERUNG("201"),

    /**
     * Delkredereversicherung
     * 
     */
    @XmlEnumValue("202")
    DELKREDEREVERSICHERUNG("202"),

    /**
     * übrige und nicht aufgegliederte Kredit- und
     * Kautionsversicherung
     * 
     */
    @XmlEnumValue("209")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_KREDIT_KAUTIONSVERSICHERUNG("209"),

    /**
     * Betriebsunterbrechungs-Versicherung
     * 
     */
    @XmlEnumValue("230")
    BETRIEBSUNTERBRECHUNGS_VERSICHERUNG("230"),

    /**
     * Beistandsleistungsversicherung
     * 
     */
    @XmlEnumValue("240")
    BEISTANDSLEISTUNGSVERSICHERUNG("240"),

    /**
     * Schutzbriefversicherung
     * 
     */
    @XmlEnumValue("241")
    SCHUTZBRIEFVERSICHERUNG("241"),

    /**
     * Sportboot-Serviceversicherung
     * 
     */
    @XmlEnumValue("242")
    SPORTBOOT_SERVICEVERSICHERUNG("242"),

    /**
     * Flugrückholkostenversicherung
     * 
     */
    @XmlEnumValue("243")
    FLUGRUECKHOLKOSTENVERSICHERUNG("243"),

    /**
     * Schutzbriefversicherung unter Einschluss der sog.
     * Mallorca-Police
     * 
     */
    @XmlEnumValue("244")
    SCHUTZBRIEFVERSICHERUNG_MIT_MALLORCA_POLICE("244"),

    /**
     * übrige und nicht aufgegliederte
     * Beistandsleistungsversicherung
     * 
     */
    @XmlEnumValue("249")
    UEBRIGE_NICHT_AUFGEGLIEDERTE_BEISTANDSLEISTUNGSVERSICHERUNG("249"),

    /**
     * Luft- und Raumfahrzeug-Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("250")
    LUFT_RAUMFAHRZEUG_HAFTPFLICHTVERSICHERUNG("250"),

    /**
     * Luftfahrt- Haftpflichtversicherung (einschließlich der
     * Luftfrachtführer-Haftpflichtversicherung)
     * 
     */
    @XmlEnumValue("251")
    LUFTFAHRT_HAFTPFLICHTVERSICHERUNG("251"),

    /**
     * Raumfahrzeug-Haftpflichtversicherung
     * 
     */
    @XmlEnumValue("252")
    RAUMFAHRZEUG_HAFTPFLICHTVERSICHERUNG("252"),

    /**
     * sonstige Sachschadenversicherung
     * 
     */
    @XmlEnumValue("291")
    SONSTIGE_SACHSCHADENVERSICHERUNG("291"),

    /**
     * Vertrauensschadenversicherung
     * 
     */
    @XmlEnumValue("296")
    VERTRAUENSSCHADENVERSICHERUNG("296"),

    /**
     * Schaden- und Unfallversicherung insgesamt
     * 
     */
    @XmlEnumValue("300")
    SCHADEN_UNFALLVERSICHERUNG_INSGESAMT("300"),

    /**
     * Kapitalanlagen
     * 
     */
    @XmlEnumValue("560")
    KAPITALANLAGEN("560"),

    /**
     * Baufinanzierungen
     * 
     */
    @XmlEnumValue("570")
    BAUFINANZIERUNGEN("570"),

    /**
     * Tierkrankenversicherung
     * 
     */
    @XmlEnumValue("684")
    TIERKRANKENVERSICHERUNG("684"),

    /**
     * Konsumentenkredit
     * 
     */
    @XmlEnumValue("590")
    KONSUMENTENKREDIT("590");
    private final String value;

    STSparte(String v) {
        value = v;
    }

    public String value() {
        return value;
    }

    public static STSparte fromValue(String v) {
        for (STSparte c: STSparte.values()) {
            if (c.value.equals(v)) {
                return c;
            }
        }
        throw new IllegalArgumentException(v);
    }

}
